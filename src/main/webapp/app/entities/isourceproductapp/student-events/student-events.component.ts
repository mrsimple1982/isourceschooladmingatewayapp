...
export class StudentEventsComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    this.studentEventsService
      ...
      .subscribe(
        (res: HttpResponse<IStudentEvents[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IStudentEvents): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInStudentEvents(): void {
    this.eventSubscriber = this.eventManager.subscribe('studentEventsListModification', () => this.loadPage());
  }

  delete(studentEvents: IStudentEvents): void {
    const modalRef = this.modalService.open(StudentEventsDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.studentEvents = studentEvents;
  }
  ...
  protected onSuccess(data: IStudentEvents[] | null, headers: HttpHeaders, page: number): void {
